...
namespace InventoryManagement
{
    public partial class CustomerForm : Form
    {
        ...
        public void LoadCustomer()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbCustomer", con);   // Creates a new command telling the database to select ALL customers from the customer table
            ...
            while (dr.Read())
            {
                ...
                dgvCustomers.Rows.Add(i, dr[0].ToString(), dr[1].ToString(), dr[2].ToString());   // Add the customer that corresponds with the row
            }
        ...
        // Method that deals with what happens when the DataGrid is clicked on
        private void dgvCustomers_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                CustomerModuleForm customerModule = new CustomerModuleForm();   // Create a new CustomerModuleForm
                customerModule.lblCustomerID.Text = dgvCustomers.Rows[e.RowIndex].Cells[1].Value.ToString();        //
                customerModule.txtCustomerName.Text = dgvCustomers.Rows[e.RowIndex].Cells[2].Value.ToString();      //  Fills the appropriate information from the
                customerModule.txtCustomerPhone.Text = dgvCustomers.Rows[e.RowIndex].Cells[3].Value.ToString();     //  datagrid

                customerModule.btnSaveCustomer.Enabled = false; // Disable the save button since we're not adding new customers
                customerModule.btnUpdateCustomer.Enabled = true;    // Since we're editing the customer, we need to update the customer
